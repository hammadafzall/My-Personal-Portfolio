...
export default function ProjectCard({ project }) {
  return (
    <div className="bg-[#1A1A1A] grid sm:grid-cols-2 items-center shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] w-full max-w-2xl max-sm:max-w-sm rounded-lg font-[sans-serif] overflow-hidden mx-auto mt-4">
      <div className="ml-2 w-full h-full lg:h-96 flex justify-center items-center">
        <Image
          src={project.image}
          className="object-cover mt-2 md:mt-0"
          width={400}
          height={100}
          alt="Project Image"
        />
      ...
      <div className="p-6 flex flex-col justify-center items-center">
        <h3 className="text-xl font-semibold text-zinc-300 md:self-start">
          {project.name}
        </h3>
        <p className="mt-3 text-sm text-gray-500 leading-relaxed text-center md:text-left">
          {project.description}
        </p>
        <p className="mt-2 text-sm text-gray-500 leading-relaxed text-center md:text-left">
          {project.tools}
        </p>
